...
class CubicInterpolator extends AbstractInterpolator {
	...
	interpolate(t) {
		...
		//m[0] = 2 * ((this.getClippedInput(k + 1) - this.getClippedInput(k - 1)) / 2);
		//m[1] = 2 * ((this.getClippedInput(k + 2) - this.getClippedInput(k + 1)) / 2);

		let result = ((((2 * t3) - (3 * t2)) + 1) * p[0]) + (((t3 - (2 * t2)) + t) * m[0]) + (((-2 * t3) + (3 * t2)) * p[1]) + ((t3 - t2) * m[1]);

		//return result;

		//result = (p[1] - p[0]) * Math.cos(t) + p[0];

		//console.log(p[0], p[1], t, result);
...
class SincFilterInterpolator extends AbstractInterpolator {
	...
	interpolate(t) {
		...
		let sum = 0;
		for (let start = (k - this.a) + 1, n = start, end = k + this.a, asc = start <= end; asc ? n <= end : n >= end; asc ? n++ : n--) {
...
var Smooth = function (arr, config) {
	...
	for (k of Object.keys(defaultConfig || {})) {
		v = defaultConfig[k];
		if (config[k] == null) {
			config[k] = v;
		}
	...
	let smoothFunc = (() => {
		...
		switch (getType(arr[0])) {
			...
			case 'Array': // vector
				...
				return t => (() => {
					const result = [];
					for (interpolator of Array.from(interpolators)) {
						result.push(interpolator.interpolate(t));
					}
					return result;
				})();
	...
	if (config.clip === 'periodic') {
		baseDomainEnd = arr.length; //after last element for periodic
	} else {
		baseDomainEnd = arr.length - 1;
	} //at last element for non-periodic

	if (!config.scaleTo) {
		config.scaleTo = baseDomainEnd;
	} //default scales to the end of the original domain for no effect
	...
	/*copy properties*/
	for (k of Object.keys(properties || {})) {
		v = properties[k];
		smoothFunc[k] = v;
	}
